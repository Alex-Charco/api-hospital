...
async function obtenerOpcionesAutorizado(req, res) {
  try {
    ...
    const formatPersona = (persona, tipo) => ({
      tipo,
      id: tipo === "PACIENTE" ? persona?.id_paciente : persona?.id_familiar,
...
async function obtenerRecetaPorCita(req, res) {
  try {
    const { id_cita } = req.query;

    if (!id_cita) {
      return res.status(400).json({ message: "id_cita es requerido" });
    }

    const receta = await recetaService.obtenerRecetaPorCita(parseInt(id_cita));

    return res.status(200).json({
      message: "Receta encontrada correctamente",
      data: receta
    });
  } catch (error) {
