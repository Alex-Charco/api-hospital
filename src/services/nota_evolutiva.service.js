...
/*async function actualizarNota(id_nota_evolutiva, nuevosDatos) {
    ...
    // Agregar la actualización del campo `link`
    if (nuevosDatos.link) {
        nota.link = nuevosDatos.link;
    }
...
async function actualizarNota(id_nota_evolutiva, nuevosDatos) { 
    const nota = await NotaEvolutiva.findByPk(id_nota_evolutiva, {
        include: [
            ...
            {
                model: Link,
                as: 'links' // ✅ Incluir relación con Link
            }
        ]
    });
...
const errorMessages = require("../utils/error_messages");
*/
// Crear una nueva nota evolutiva v1
...
// Crear una nueva nota evolutiva v2
/*async function crearNota(data, transaction) {
    try {
        return await NotaEvolutiva.create(data, { transaction });
    } catch (error) {
